class UsersHasIngredientsController < ApplicationController
  ...
  def index
    @users_has_ingredient = UsersHasIngredient.all
  end
  ...
  def new
    @users_has_ingredient = UsersHasIngredient.new
     @users_has_ingredient.users_id = current_user.id 
  end
  ...
  def create
    ...
    respond_to do |format|
      if @users_has_ingredient.save
        format.html { redirect_to @users_has_ingredient, notice: 'Users new ingredient was successfully created.' }
  ...
  private
    ...
    def set_users_has_ingredient
      @users_has_ingredient = UsersHasIngredient.find(params[:users_id],params[:ingredient_id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def users_has_ingredient_params
      params.require(:users_has_ingredient).permit(:users_id, :ingredient_id)
    end
end
